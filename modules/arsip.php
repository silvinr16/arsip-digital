...
// -----------------------------
// SOLUSI NOT FOUND: Dapatkan nama file secara dinamis
// -----------------------------
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah'])) {
    ...
    if (empty($no_agenda) || empty($tgl_terima) || empty($asal_surat) || empty($nomor_surat)) {
        header("Location: {$current_file}?msg=error&detail=Input tidak lengkap&limit={$limit}&year={$year_filter}");
        exit;
    }
    ...
    if ($file_data['error'] && !str_contains($file_data['error'], 'Tidak ada file')) {
        header("Location: {$current_file}?msg=error&detail=".urlencode($file_data['error'])."&limit={$limit}&year={$year_filter}");
        exit;
    }
    ...
    try {
        ...
        header("Location: {$current_file}?msg=added&limit={$limit}&page={$page}&year={$year_filter}");
    } catch (Exception $e) {
        error_log("🔥 Gagal simpan ke database: " . $e->getMessage());
        header("Location: {$current_file}?msg=error&detail=Gagal simpan ke database (Cek ukuran data!)&limit={$limit}&year={$year_filter}");
    }
    exit;
}
...
if (isset($_GET['delete'])) {
    ...
    $msg_status = 'deleted';
    header("Location: {$current_file}?msg={$msg_status}&limit={$limit}&page={$page}&year={$year_filter}");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    if ($id === null) {
        header("Location: {$current_file}?msg=error&detail=ID tidak ditemukan&year={$year_filter}");
        exit;
    }
    ...
    if (!empty($_FILES['file']['name']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($file_data['error'] && !str_contains($file_data['error'], 'Tidak ada file')) {
            header("Location: {$current_file}?msg=error&detail=".urlencode($file_data['error'])."&limit={$limit}&page={$page}&year={$year_filter}");
            exit;
        }
    ...
    try {
        ...
        header("Location: {$current_file}?msg=updated&limit={$limit}&page={$page}&year={$year_filter}");
    } catch (Exception $e) {
        error_log("🔥 Gagal update ke database: " . $e->getMessage());
        header("Location: {$current_file}?msg=error&detail=Gagal update ke database (Cek ukuran data!)&limit={$limit}&page={$page}&year={$year_filter}");
    }
    exit;
}
...
if (is_array($all)) {
    foreach ($all as $key => $val) {
        ...
        // Ekstrak tahun dari tgl_terima atau created_at
        $date_field = !empty($item['tgl_terima']) ? $item['tgl_terima'] : $item['created_at'];
        if ($date_field) {
            // Coba ekstrak tahun dari format tanggal
            if (preg_match('/(\d{4})/', $date_field, $matches)) {
                $year = $matches[1];
...
if ($year_filter !== '') {
    $filtered = [];
    foreach ($items as $it) {
        $date_field = !empty($it['tgl_terima']) ? $it['tgl_terima'] : $it['created_at'];
        if ($date_field && preg_match('/(\d{4})/', $date_field, $matches)) {
            $year = $matches[1];
            if ($year == $year_filter) {
                $filtered[] = $it;
            }
...
<div class="container-fluid px-0">
  <!-- Card Header dengan Judul dan Fitur Pencarian -->
  <div class="card shadow-sm mb-4 border-0">
    <div class="card-header bg-gradient-primary text-white py-3">
      <div class="row align-items-center">
        <div class="col-md-6">
          <h4 class="mb-0 d-flex align-items-center">
            <i class="bi bi-archive me-2"></i> Arsip Digital
          </h4>
        </div>
        ...
        <div class="col-md-4">
          <form method="get" class="d-flex">
            ...
            <div class="input-group">
              <input type="text" name="search" value="<?= htmlspecialchars($search ?? '', ENT_QUOTES, 'UTF-8') ?>" class="form-control" placeholder="Cari data arsip...">
          ...
          <form method="get" class="d-flex">
            ...
            <select name="year" class="form-select" onchange="this.form.submit()">
              ...
              <?php foreach ($available_years as $year): ?>
                <option value="<?= htmlspecialchars($year) ?>" <?= ($year_filter == $year) ? 'selected' : '' ?>><?= htmlspecialchars($year) ?></option>
              <?php endforeach; ?>
            </select>
          </form>
        </div>
          ...
          <form method="get" class="d-flex">
            ...
            <select name="limit" class="form-select" onchange="this.form.submit()">
              <?php foreach ($allowed_limits as $opt): ?>
                <option value="<?= htmlspecialchars($opt) ?>" <?= ($limit == $opt) ? 'selected' : '' ?>><?= htmlspecialchars($opt) ?> data/halaman</option>
              <?php endforeach; ?>
            </select>
          </form>
        </div>
  ...
  .bg-gradient-primary {
    background: linear-gradient(135deg,rgb(55, 105, 243) 0%, #2575fc 100%);
  }
  </style>

  <!-- Card Tabel Data dengan Desain yang Lebih Menonjol -->
  <div class="card shadow-lg border-0">
    <div class="card-header bg-white border-bottom py-3">
      <div class="d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-primary fw-bold">
          <i class="bi bi-table me-2"></i> Data Arsip Digital
          <?php if ($year_filter !== ''): ?>
            <span class="badge bg-secondary ms-2">Tahun <?= htmlspecialchars($year_filter) ?></span>
          <?php endif; ?>
        </h5>
    ...
    <div class="card-body p-0">
      <div class="table-responsive">
        <table class="table table-sm table-striped table-hover align-middle mb-0" style="font-size:13px; table-layout:fixed; width:100%;">
          ...
          <tbody class="fw-semibold">
                ...
                <tr>
                  ...
                  <td class="text-center">
                    <div class="btn-group" role="group">
                      ...
                      <a href="?delete=<?= htmlspecialchars($row['id']) ?>&page=<?= htmlspecialchars($page) ?>&limit=<?= htmlspecialchars($limit) ?>&search=<?= urlencode($search) ?>&year=<?= htmlspecialchars($year_filter) ?>" 
                ...
                <div class="modal-content shadow border-0 rounded-3">
                  ...
                  <div class="modal-header py-2 bg-primary text-white rounded-top">
                    <h6 class="modal-title fw-semibold d-flex align-items-center mb-0">
                      <i class="bi bi-envelope-paper me-2"></i> Edit Arsip Digital
                    </h6>
                        ...
                        <div class="card-body">
                          ...
                          <div class="row g-3">
                            <div class="col-12">
                              ...
                              <!-- PERBAIKAN: Tampilkan file yang ada saat ini dengan tombol preview tanpa tombol unduh -->
                              <?php if (!empty($row['file_base64'])): 
                                  $data_uri = 'data:' . htmlspecialchars($row['file_mime'] ?? 'application/octet-stream') . ';base64,' . $row['file_base64'];
            ...
            <!-- PERBAIKAN: Modal Preview File di dalam Modal Edit tanpa tombol unduh -->
            <?php if (!empty($row['file_base64'])): ?>
          ...
          <?php else: ?>
            <tr>
              <td colspan="13" class="text-center text-muted py-5">
                ...
                <?php if ($year_filter !== ''): ?>
                  <p class="mt-1 mb-0">Untuk tahun <?= htmlspecialchars($year_filter) ?></p>
                <?php endif; ?>
              </td>
            </tr>
          <?php endif; ?>
  ...
  <!-- Card Footer dengan Pagination dan Informasi Data -->
      ...
      <div class="row align-items-center">
        <div class="col-md-6">
          <p class="small text-muted mb-0">
            ...
            <?php if ($year_filter !== ''): ?>
              <span class="badge bg-secondary ms-1">Tahun <?= htmlspecialchars($year_filter) ?></span>
            <?php endif; ?>
          </p>
        </div>
        <div class="col-md-6">
          <nav>
            <ul class="pagination pagination-sm justify-content-end mb-0">
              ...
              if ($page > 1): ?>
                <li class="page-item"><a class="page-link" href="?page=1&limit=<?= htmlspecialchars($limit) ?>&search=<?= $search_url ?>&year=<?= htmlspecialchars($year_filter) ?>">« First</a></li>
                <li class="page-item"><a class="page-link" href="?page=<?= htmlspecialchars($page-1) ?>&limit=<?= htmlspecialchars($limit) ?>&search=<?= $search_url ?>&year=<?= htmlspecialchars($year_filter) ?>">‹ Prev</a></li>
              <?php endif; ?>
              ...
              if ($start > 1) {
                  echo '<li class="page-item"><a class="page-link" href="?page=1&limit='.htmlspecialchars($limit).'&search='.$search_url.'&year='.htmlspecialchars($year_filter).'">1</a></li>';
                  if ($start > 2) echo '<li class="page-item disabled"><span class="page-link">...</span></li>';
              }

              for ($i=$start; $i<=$end; $i++): ?>
                <li class="page-item <?= ($i==$page)?'active':'' ?>">
                  <a class="page-link" href="?page=<?= htmlspecialchars($i) ?>&limit=<?= htmlspecialchars($limit) ?>&search=<?= $search_url ?>&year=<?= htmlspecialchars($year_filter) ?>"><?= htmlspecialchars($i) ?></a>
                </li>
              <?php endfor;

              if ($end < $total_pages) {
                  if ($end < $total_pages - 1) echo '<li class="page-item disabled"><span class="page-link">...</span></li>';
                  echo '<li class="page-item"><a class="page-link" href="?page='.htmlspecialchars($total_pages).'&limit='.htmlspecialchars($limit).'&search='.$search_url.'&year='.htmlspecialchars($year_filter).'">'.htmlspecialchars($total_pages).'</a></li>';
              }
              ?>

              <?php if ($page < $total_pages): ?>
                <li class="page-item"><a class="page-link" href="?page=<?= htmlspecialchars($page+1) ?>&limit=<?= htmlspecialchars($limit) ?>&search=<?= $search_url ?>&year=<?= htmlspecialchars($year_filter) ?>">Next ›</a></li>
                <li class="page-item"><a class="page-link" href="?page=<?= htmlspecialchars($total_pages) ?>&limit=<?= htmlspecialchars($limit) ?>&search=<?= $search_url ?>&year=<?= htmlspecialchars($year_filter) ?>">Last »</a></li>
              <?php endif; ?>
            </ul>
          </nav>
        </div>
    ...
    <div class="modal-content border-0 shadow-lg rounded-3">
      <div class="modal-header py-3 bg-success text-white rounded-top">
        <h5 class="modal-title fw-semibold d-flex align-items-center mb-0" id="modalTambahLabel">
          <i class="bi bi-envelope-paper me-2"></i> Tambah Arsip Digital
        </h5>
